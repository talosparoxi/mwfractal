...
ProgramOptions::ProgramOptions( int argc, char **argv ) {
    ...
    model.add_options()
        ...
        ( "c_real", bpo::value<float>(&this->cr)->default_value(-0.2321), "Real part of c" )
        ( "c_imag", bpo::value<float>(&this->ci)->default_value(-0.835), "Imaginary part of c" )
        ( "min_real", bpo::value<float>(&this->min_re)->default_value(-1), "" )
        ( "max_real", bpo::value<float>(&this->max_re)->default_value(1), "" )
    ...
    graphics.add_options()
        ( "width", bpo::value<int>(&this->width)->default_value(1080), "Output image width" )
        ( "height", bpo::value<int>(&this->height)->default_value(900), "Output image height" )
